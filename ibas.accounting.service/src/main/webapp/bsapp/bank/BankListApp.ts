...
namespace accounting {
    export namespace app {
        /** 列表应用-银行 */
        export class BankListApp extends ibas.BOListApplication<IBankListView, bo.Bank> {
            /** 应用标识 */
            static APPLICATION_ID: string = "62cf6385-456d-43f8-8985-28be458c73d8";
            /** 应用名称 */
            static APPLICATION_NAME: string = "accounting_app_bank_list";
            /** 业务对象编码 */
            static BUSINESS_OBJECT_CODE: string = bo.Bank.BUSINESS_OBJECT_CODE;
            /** 构造函数 */
            constructor() {
                super();
                this.id = BankListApp.APPLICATION_ID;
                this.name = BankListApp.APPLICATION_NAME;
                this.boCode = BankListApp.BUSINESS_OBJECT_CODE;
            ...
            protected viewData(data: bo.Bank): void {
                ...
                let app: BankViewApp = new BankViewApp();
                app.navigation = this.navigation;
                app.viewShower = this.viewShower;
                app.run(data);
            ...
            protected deleteData(data: bo.Bank | bo.Bank[]): void {
                let beDeleteds: ibas.IList<bo.Bank> = ibas.arrays.create(data);
                // 没有选择删除的对象
        ...
        /** 视图-银行 */
        export interface IBankListView extends ibas.IBOListView {
            /** 编辑数据事件，参数：编辑对象 */
