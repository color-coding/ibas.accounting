                        ...
                        for (let source of sources) {
                            ...
                            if (targetCurrency === config.currency("LOCAL")) {
                                ...
                                if (ibas.objects.isNull(rate)) {
                                    throw new Error(ibas.i18n.prop("accounting_currency_exchange_not_found_rate", rateDate, targetCurrency));
                                }
                                results[sources.indexOf(source)] = {
                                    currency: targetCurrency,
                            ...
                            if (!ibas.strings.equalsIgnoreCase(target.currency, config.currency("LOCAL"))) {
                                ...
                                if (ibas.objects.isNull(rate)) {
                                    throw new Error(ibas.i18n.prop("accounting_currency_exchange_not_found_rate", rateDate, target.currency));
                                }
